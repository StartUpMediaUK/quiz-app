...
const buttonVariants = tv({
  ...
  variants: {
    variant: {
      primary: "bg-primary text-primary-foreground shadow-xs hover:bg-primary/90 border-transparent",
    ...
    size: {
      ...
      xs: "h-6 rouned-md px-2 text-xs",
      sm: "h-9 rounded-md px-3",
      lg: "h-11 rounded-md px-8",
      xl: "h-15 rounded-md px-10",
      "2xl": "h-20 rounded-md px-16",
      icon: "h-10 w-10",
      ghost: "h-10 p-0",
    },
  },
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ asChild, isLoading = false, loadingText, className, disabled, variant, size, children, ...props }: ButtonProps, forwardedRef) => {
