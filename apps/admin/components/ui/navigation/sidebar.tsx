  ...
  return (
    <>
        ...
        <aside className="flex grow flex-col gap-y-4 overflow-y-auto whitespace-nowrap px-3 py-4">
          <div>
            <div className="flex items-center gap-x-1.5">
              <button className="group inline-flex rounded-md p-2 hover:bg-gray-200/50 hover:dark:bg-gray-900" onClick={toggleSidebar}>
                {isCollapsed ? (
                  <PanelRightClose
              ...
              <span
                className={cn("text-sm font-semibold text-gray-900 transition-opacity dark:text-gray-50", isCollapsed ? "opacity-0" : "opacity-100")}>
          ...
          <nav aria-label="core navigation links" className="flex flex-1 flex-col space-y-10">
                    ...
                    {isCollapsed ? (
                      <Tooltip side="right" content={item.name} sideOffset={6} showArrow={false} className="z-[999]">
                        <Link
                          ...
                          {item.icon.type === "icon" ? (
                            <item.icon.avatar className="size-5 shrink-0" aria-hidden="true" />
                          ) : item.icon.type === "image" ? (
                            item.icon.avatar
                          ) : (
                            <Image src={item.icon.url} width={36} height={36} alt={item.name} />
                          )}
                        </Link>
                      </Tooltip>
                    ) : (
            ...
            <div>
              ...
              <ul role="list" className="mt-1 space-y-2">
                <li>
                  {isCollapsed ? (
                    <Tooltip side="right" content="Onboarding" sideOffset={6} showArrow={false} className="z-[999]">
                      <Link
                        href={siteConfig.baseLinks.settings}
                        className={cn(
                          isActive("/onboarding") ? "text-blue-600 dark:text-blue-500" : "text-gray-700 dark:text-gray-300",
                          "inline-flex items-center rounded-md p-2 text-sm font-medium transition hover:bg-gray-200/50 hover:dark:bg-gray-900",
                          focusRing
                        )}>
      ...
      <div className="sticky top-0 z-40 flex h-16 shrink-0 items-center justify-between border-b border-gray-200 bg-white px-4 shadow-sm sm:px-6 lg:hidden dark:border-gray-800 dark:bg-gray-950">
        <span className={cn("font-semibold text-gray-900 sm:text-sm dark:text-gray-50", isCollapsed ? "opacity-0" : "opacity-100")}>
          <Link aria-label="Home Link" href="/" className="flex items-center justify-center">
            {siteConfig.name}
          </Link>
        </span>
