...
import { upsertCanvasPage } from "@/db/queries/canvas";
import { type DeviceTypes } from "@/lib/types/editor";
import { cn } from "@workspace/ui/lib/utils";

interface CanvasEditorNavigationProps {
  db: CanvasDB;
  funnelId: string;
  subAccountId: string;
...
const CanvasEditorNavigation: React.FC<CanvasEditorNavigationProps> = ({
  db,
  funnelId,
  funnelPageDetails,
  subAccountId,
  ...
  const handleBlurTitleChange = async (
    ...
    if (event.target.value) {
      await upsertCanvasPage({db,subAccountId, canvasPageId: funnelPageDetails.id, canvasPage: {
        ...funnelPageDetails,
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await upsertCanvasPage({db, subAccountId, canvasPageId: funnelPageDetails.id, canvasPage: {
        ...funnelPageDetails,
        content,
      }});
...
export default CanvasEditorNavigation;
